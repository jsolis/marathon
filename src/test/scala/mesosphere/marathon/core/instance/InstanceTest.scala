...
class InstanceTest extends UnitTest {
  ...
  "The instance startedAt timestamp" when {
    ...
    "all tasks are started" should {
      ...
      "give the youngest" in { instance.startedAt should be(Some(f.clock.now)) }
    }
    ...
    "no task is started" should {
      ...
      "give not startedAt timestamp" in { instance.startedAt should not be 'defined }
    }
    ...
    "one task is not start but another is" should {
      ...
      "give the youngest of the started tasks" in { instance.startedAt should be(Some(f.clock.now - 1.hour)) }
    }
  }
