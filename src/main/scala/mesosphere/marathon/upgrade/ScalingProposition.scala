...
private[this] object SortHelper {
  ...
  def startedAt(instance: Instance): Option[Timestamp] = {
    ...
    // Turn Seq[Option[_]] inside out to Option[Seq[_]]
    taskStartedAts.foldLeft(Option.empty[Seq[Timestamp]]) {
      case (Some(seq), Some(n)) => Some(seq.:+(n))
      case (None, Some(n: Timestamp)) => Some(Seq(n))
      case (c, None) => c
    }
      .map(_.max) // Take max startedAt timestamp if any
  }
